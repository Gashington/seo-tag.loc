...
class phpMorphy_Graminfo_Mem extends phpMorphy_Graminfo {
    ...
    function readGramInfoHeader($offset) {
        ...
        $result = unpack(
                'vid/vfreq/vforms_count/vpacked_forms_count/vancodes_count/vancodes_offset/vancodes_map_offset/vaffixes_offset/vaffixes_size/vbase_size', $GLOBALS['__phpmorphy_substr']($__mem, $offset, 20)
        );
    ...
    function readAllPartOfSpeech() {
        ...
        foreach ($this->readSectionIndexAsSize($this->header['poses_index_offset'], $this->header['poses_count'], $this->header['poses_size']) as $size) {
            ...
            $res = unpack(
                    'vid/Cis_predict', $GLOBALS['__phpmorphy_substr']($__mem, $offset, 3)
            );
    ...
    function readAllGrammems() {
        ...
        foreach ($this->readSectionIndexAsSize($this->header['grammems_index_offset'], $this->header['grammems_count'], $this->header['grammems_size']) as $size) {
            ...
            $res = unpack(
                    'vid/Cshift', $GLOBALS['__phpmorphy_substr']($__mem, $offset, 3)
            );
